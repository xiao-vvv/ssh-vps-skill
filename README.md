...
> **初次使用提示：密钥、SSH 端口、别名都可以按你自己的需要设置，没有强制值：**
> - **密钥/公钥**：默认用 `~/.ssh/vps`。想用你已有的密钥，把它放到这个路径，或设环境变量 `VPS_KEY_PATH=/你的/密钥` 指过去即可。
> - **SSH 端口**：连接端口默认 `22`（新鸡通常就是 22，可用 `--port` 覆盖）；`port` 改端口的默认目标是 `20266`，可 `ssh-vps port <别名> <你要的端口>`，或用 `VPS_HARDEN_PORT` 改默认。
> - **别名**：`add <别名> <host>` 里的别名你随便起（字母 / 数字 / `.` / `_` / `-`），起成自己记得住的名字就行，比如 `tokyo-1`、`hk-bgp`。
